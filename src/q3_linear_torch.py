...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        Args:
            state: (torch tensor)
                shape = (batch_size, nchannels x config.state_history,  img height, img width)
            network: (str)
    ...
    def update_target(self):
        ...
        params = self.q_network.state_dict()
        self.target_network.load_state_dict(params)
        ##############################################################
        ######################## END YOUR CODE #######################


    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
                    actions : Tensor, rewards: Tensor, done_mask: Tensor, state: Tensor, next_state: Tensor) -> Tensor:
        """
        ...
        state_action_vals = self.get_q_values(state, 'q_network').gather(1, actions)
        state_action_vals = state_action_vals.squeeze(-1)
        next_state_action = self.get_q_values(next_state, 'q_network').max(1)[1]
        next_state_action = next_state_action.unsqueeze(-1)
        next_state_vals = self.get_q_values(next_state, 'target').gather(1, next_state_action).squeeze(-1)
        
        exp_sa_vals = next_state_vals.detach()*gamma*(1-done_mask) + rewards
